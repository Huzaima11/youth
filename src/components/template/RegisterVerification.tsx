...
export default function RegisterVerification({
    ...
    const [vissibleProfile, setVissibleProfile] = useState(false);
    const [receivedToken, setReceivedToken] = useState<string | null>(null);


    // auth hook
    const { states, handleRegisterOtp, registerUser } = useAuth();
    const { loading, error } = states.otp; // assume states.verify
    const { error: resendError } = states.register;
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            const response = await handleRegisterOtp(values.otp);
            states.register.error = null;

            if (response?.success) {
                setReceivedToken(response?.token);
                setVissibleProfile(true);
                states.register.error = null;
            }
        },
    });
    ...
    return (
        <>
            {
                vissibleProfile ?
                    <Profile profileToken={receivedToken} />
                    :
